...
export function buildLoaders(options: BuildOptions): ModuleOptions["rules"] {
  ...
  const assetLoader = {
    // данный лоадер нужен для работы с графическими файлами разных типов в проекте
    test: /\.(png|jpg|jpeg|gif)$/i,
    type: "asset/resource",
  };

  const svgrLoader = {
    // данный лоадер преобразовывает svg картинки в реакт компоненты, которые можно настраивать через пропсы
    test: /\.svg$/i,
    issuer: /\.[jt]sx?$/,
    use: [
      {
  ...
  const styleLoader = {
    test: /\.s[ac]ss$/i,
    use: [
      // если лоадеров несколько то можно подключать их массивом, все эти лоадеры устанавливаются как npm пакеты, порядок важен для последовательно преобразования кода
    ...
    use: [
      {
        ...
        options: {
          ...
          getCustomTransformers: () => ({
            // данный метод нужен для валидации типов для обновления реакта в runtime без обновления страницы
